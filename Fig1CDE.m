...
subplot(2,1,2);
%plotschnitzme(s,'hrst','lengthMicrons',[],'k-');
...
% [pk,lk] = findpeaks((L_smooth(:,9)),T(:,9));
% plot(T(:,9),L_smooth(:,9),'c-',lk,pk,'mo','LineWidth',2, 'MarkerSize',8);
% hold on
% [pk,lk] = findpeaks((L_smooth(:,59)),T(:,59));
% plot(T(:,59),L_smooth(:,59),'m-',lk,pk,'mo','LineWidth',2, 'MarkerSize',8);
nightmodeon(schnitz1{18,2},12,1);
xlabel('Time in LL (h)','fontweight','bold')
ylabel('Cell length (\mum)','fontweight','bold')
ylim([2 10])
...
for j = 2:length(s_ML)
    for i = 1:length(s_ML{j,2})
        if s_ML{j,2}(i).E ~= 0 && s_ML{j,2}(i).P ~= 0 % find cells with complete cell cycle
            ...
            t1 = [s_ML{j,2}(i).hrs-max(s_ML{j,2}(i).hrs),s_ML{j,2}(A).hrs-min(s_ML{j,2}(A).hrs)]; % adjust the time according to time of division
            y1 = [s_ML{j,2}(i).MYs_det,s_ML{j,2}(A).MYs_det];
            
            
            t2 = [s_ML{j,2}(i).hrs-max(s_ML{j,2}(i).hrs),s_ML{j,2}(B).hrs-min(s_ML{j,2}(B).hrs)];
...
%saveas(gcf,['Fig5_2D'],'fig');saveas(gcf,['Fig5_2D'],'svg');
...
for j = 2:length(s_delkaiBC)
    for i = 1:length(s_delkaiBC{j,2})
        if s_delkaiBC{j,2}(i).E ~= 0 && s_delkaiBC{j,2}(i).P ~= 0
            ...
            t1 = [s_delkaiBC{j,2}(i).hrs-max(s_delkaiBC{j,2}(i).hrs),s_delkaiBC{j,2}(A).hrs-min(s_delkaiBC{j,2}(A).hrs)];
            y1 = [s_delkaiBC{j,2}(i).MYs_det,s_delkaiBC{j,2}(A).MYs_det];
            
            t2 = [s_delkaiBC{j,2}(i).hrs-max(s_delkaiBC{j,2}(i).hrs),s_delkaiBC{j,2}(B).hrs-min(s_delkaiBC{j,2}(B).hrs)];
...
plot(xs,meanyperx,'-','Color',[1,0.9,0],'LineWidth',4)
...
%saveas(gcf,['Fig5_2E'],'fig');saveas(gcf,['Fig5_2E'],'svg');