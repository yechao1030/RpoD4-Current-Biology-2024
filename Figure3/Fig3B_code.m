...
%% Medium light delkaiBC
sall_delkaiBC = [];

for k = 2:size(s,1)
s1 = s_delkaiBC{k,2};
sall_delkaiBC = cat(2,sall_delkaiBC,s1);
end
...
for i = 2:size(s,1)
    ...
    [L salt] = schnitzTables(s{i,2},'lengthMicrons_smooth');
    [T salt] = schnitzTables(s{i,2},'hrst');
    Y_delkaiBC = catpad(2,Y_delkaiBC,Y);
    L_delkaiBC = catpad(2,L_delkaiBC,L);
    T_delkaiBC = catpad(2,T_delkaiBC,T);
    salt_delkaiBC = catpad(2,salt_delkaiBC,salt);
end
...
cmp = parula(16);
cmp = cat(1,flipud(cmp),cmp);
for k = 1:size(T_delkaiBC,2)
    y = rmmissing(Y_delkaiBC(:,k));
    t = rmmissing(T_delkaiBC(:,k));
%     f = polyfit(t,y,2);
%     ypol = y-t.^2*f(1)-t*f(2)-f(3);
    %ypol = y-t*f(1)-f(2);
    [pks locs w p] = findpeaks(y,t,'MinPeakProminence',100);
    ...
    for n = 1:length(locs)
        ...
        idx = find(CT==ct);
        %plot(t([max(idx0(n)-9,1):min(idx0(n)+9,length(y))]),y([max(idx0(n)-9,1):min(idx0(n)+9,length(y))]),'color',[cmp(idx,:)])
        plot(t([max(idx0(n)-9,1):min(idx0(n)+9,length(y))]),y([max(idx0(n)-9,1):min(idx0(n)+9,length(y))])./meany,'color',[cmp(idx,:)])% normalise by mean pulse height
...
for k = 1:size(T_wt,2)
    ...
    %ypol = y-t.^2*f(1)-t*f(2)-f(3);
    if size(t,1)>=3
    [pks locs w p] = findpeaks(y,t,'MinPeakProminence',150);
    [pks0 idx0] = findpeaks(y,'MinPeakProminence',150);
...
xlim([0 120])
colormap(cmp)
colorbar('Ticks',[0:1/4:1],'TickLabels',{'0','5.25','11.25','17.25','23.25'})
xlabel('Time in LL (h)','FontWeight','bold')
ylabel('P_{\itrpoD4}-EYFP pulse (a.u.)','FontWeight','bold')
title('GMM')